...
/// subtracting it.
///
/// This view relies on incremental drawing: `draw(_:)` clears and repaints only the
/// invalidated rects and trusts that pixels outside them survive from earlier passes.
/// That holds for a non-layer-backed view (the default). Do not set `wantsLayer = true`
/// here without switching to a redraw model that repaints the whole dirty region, or
/// marks outside the invalidated rects would silently vanish.
@MainActor
